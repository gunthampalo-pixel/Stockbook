...
- หน้าแรกเป็น `Home / Command Center` สำหรับดูภาพรวมระบบ สถานะ Supabase/Gemini และแผนที่ข้อมูล
- มีหน้า `Admin` สำหรับ reload Supabase, test Supabase, test Gemini, เปิด API settings และ clear device cache
...
### 2026-07-19 14:55 ICT - Codex

**เป้าหมาย**

ทำเว็บ Project stock ให้ออกมาตรง blueprint ที่ผู้ใช้สรุป: หน้าแรกต้องบอกโครงระบบได้, เมนูต้องชัด, หลังบ้านต้องบอกว่าอะไรต่อ Supabase/Gemini/TradingView และอะไรยังเป็นงานต่อ

**สิ่งที่ทำ**

- เพิ่มหน้า `Home / Command Center` เป็นหน้าแรกของเว็บ
- เพิ่มหน้า `Admin / Settings` สำหรับ reload Supabase, test Supabase, test Gemini, clear device cache และเปิด API key modal
- จัดเมนูใหม่เป็นกลุ่ม `Command`, `Stocks`, `Valuation`, `Knowledge`, `System`
- เอาปุ่ม import/export ออกจาก header เพื่อไม่ให้สื่อว่าข้อมูลหลักอยู่ในไฟล์เครื่อง
- เพิ่มตารางแผนที่ข้อมูลว่าหุ้น/notes/knowledge/fair value/extracted knowledge/AI/chart ใช้ source ไหน
- ปรับ `PROJECT_BLUEPRINT.md` ให้โทนชัดขึ้นโดยไม่ย้ำเรื่องงานผิดแชทเกินจำเป็น
- อัปเดต `README.md` ให้ระบุหน้าใช้งานหลัก
...
- ตรวจ syntax ของ inline JavaScript ผ่าน

**สถานะ**

- พร้อม build/push/deploy

**งานต่อ**

- ต่อ API ราคาหุ้น/งบการเงินจริงให้ชัดว่าจะใช้ provider ไหน
- ทำ schema เพิ่มสำหรับ watchlist/fair value/options ถ้าต้องการแยกจาก `stocks` และ `stock_research_knowledge_documents`
- ทำ server-side AI proxy เพื่อไม่ต้องกรอก Gemini key ราย browser
